...
class IncedentsController < ApplicationController
  ...
  def index
    if cookies[:viewmode] == 'tree'
      @incedents = get_incedents(false).nested_set.order('updated_at DESC, finish_at, priority_id DESC, status_id DESC').paginate(page: params[:page], per_page: @per_page)
    else
      @incedents = get_incedents(false).order('updated_at DESC, finish_at, priority_id DESC, status_id DESC').paginate(page: params[:page], per_page: @per_page)
    end
    ...
    respond_to do |format|
      ...
      format.xls {
        @incedents = get_incedents(false).order('created_at DESC, finish_at, priority_id DESC, status_id DESC')
  ...
  def observe
    if cookies[:viewmode] == 'tree'
      @incedents = get_incedents(false).nested_set.by_observer(@current_user).order('updated_at DESC, finish_at, priority_id DESC, status_id DESC').paginate(page: params[:page], per_page: @per_page)
    else
      @incedents = get_incedents(false).by_observer(@current_user).order('updated_at DESC, finish_at, priority_id DESC, status_id DESC').paginate(page: params[:page], per_page: @per_page)
    end
  ...
  def onreview
    if cookies[:viewmode] == 'tree'
      @incedents = get_incedents(false).nested_set.by_reviewer(@current_user).not_reviewed(@current_user).order('updated_at DESC, finish_at, priority_id DESC, status_id DESC').paginate(page: params[:page], per_page: @per_page)
    else
      @incedents = get_incedents(false).by_reviewer(@current_user).not_reviewed(@current_user).order('updated_at DESC, finish_at, priority_id DESC, status_id DESC').paginate(page: params[:page], per_page: @per_page)
    end
  ...
  def work
    if params[:incedent] and !params[:incedent][:work_reason].empty? and !params[:incedent][:worker_ids].empty?
      ...
      if params[:incedent][:worker_ids]
        (@incedent.worker_ids - params[:incedent][:worker_ids]).each do |worker_id|
            @incedent.delete_worker User.find(worker_id)
        end
